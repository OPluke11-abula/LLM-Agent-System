...
The Conductor Trace panel now includes Workflow Gate and Structural Memory surfaces for workflow stage, checkpoint reference, evidence-ref count, review-gate status, code-graph ref count, changed-file fanout, impacted symbols, linked tests, and security-relevant paths. It consumes the same `workflow_stage_id`, `workflow_checkpoint_ref`, `evidence_refs`, `code_graph_refs`, and `impact_summary` metadata already emitted in topology conductor traces while keeping raw evidence in local trace files.
...
Conductor Trace 面板現在新增 Workflow Gate 與 Structural Memory surfaces，可顯示 workflow stage、checkpoint reference、evidence-ref 數量、review-gate status、code-graph ref count、changed-file fanout、impacted symbols、linked tests 與 security-relevant paths。它直接消費 topology conductor trace 已發出的 `workflow_stage_id`、`workflow_checkpoint_ref`、`evidence_refs`、`code_graph_refs` 與 `impact_summary` metadata，同時把 raw evidence 保留在本地 trace files。
