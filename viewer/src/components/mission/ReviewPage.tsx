...
import { GateStatus, type Mission, type MissionTransitionPage } from "../../generated/missionContracts";
...
export function ReviewPage() {
  ...
  const gateRows = (mission.required_verification ?? []).map((gate) => {
    ...
    const pass = record?.status === GateStatus.PASSED && refsValid && linkedEvidence.every((item) => item?.verification_status === GateStatus.PASSED);
    return { gate, record, refs, linkedEvidence, pass };
  });
