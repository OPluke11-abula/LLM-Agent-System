...
export function ReviewPage() {
  ...
  async function loadMoreHistory(): Promise<void> {
    ...
    try {
      const next = await missionApi.history(decodedMissionId, { offset: history.next_offset, signal: undefined });
      setHistory({ ...next, items: [...history.items, ...next.items] });
    } catch (cause: unknown) {
      setError(cause instanceof MissionApiError ? cause.message : "More transition history could not be loaded");
  ...
  const unresolved = verifications.filter((gate) => gate.status !== "passed");
