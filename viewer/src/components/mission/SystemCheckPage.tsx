...
import type { MissionSystemCapabilities } from "../../generated/missionContracts";
...
type CheckState = "idle" | "checking" | "unreachable" | "unauthorized" | "authenticated_compatible" | "authenticated_storage_unavailable" | "schema_mismatch" | "server_error" | "auth_unavailable";
...
function statusTone(state: CheckState): "success" | "warning" | "danger" | "neutral" {
  ...
  if (state === "authenticated_storage_unavailable" || state === "schema_mismatch") return "warning";
  return "danger";
}
...
export function SystemCheckPage() {
  ...
  const check = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await missionApi.checkSystem();
      setStatus(200);
      setCapabilities(result);
      setState(result.schema_compatible ? result.mission_store_available ? "authenticated_compatible" : "authenticated_storage_unavailable" : "schema_mismatch");
    } catch (cause: unknown) {
  ...
  return <section className="flex h-full min-h-0 flex-col gap-5 overflow-auto pb-6">
    ...
    <Surface elevated className="max-w-3xl p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold t1">Mission API capability contract</p><p className="mt-1 font-mono text-xs t3">GET /v1/system/capabilities</p></div><StatusBadge tone={statusTone(state)}>{statusLabel(state)}</StatusBadge></div>{error && <p className="mt-4 text-sm" style={{ color: "var(--danger)" }} role="alert">{error}</p>}<div className="mt-5 grid gap-2 border-t pt-4 text-sm" style={{ borderColor: "var(--border-c)" }}><div className="flex justify-between gap-4"><span className="t2">HTTP status</span><span className="font-mono t1">{status ?? "—"}</span></div>{capabilities && <><div className="flex justify-between gap-4"><span className="t2">Workspace readiness</span><span className="t1">{capabilities.workspace_root_available ? "available" : "unavailable"}</span></div><div className="flex justify-between gap-4"><span className="t2">Mission store</span><span className="t1">{capabilities.mission_store_available ? "available" : "unavailable"}</span></div><div className="flex justify-between gap-4"><span className="t2">Schema</span><span className="font-mono text-xs t1">{capabilities.contract_schema_version} / Viewer {capabilities.viewer_expected_schema_version}</span></div><div className="flex justify-between gap-4"><span className="t2">Provider configuration</span><span className="t1">{capabilities.provider_configuration.replaceAll("_", " ")}</span></div></>}</div><Button className="mt-5" onClick={() => void check()} disabled={state === "checking"}>{state === "checking" ? "Checking…" : "Run check again"}</Button>{state === "authenticated_compatible" && <LinkButton to="/missions" variant="primary" className="ml-3">Continue to Missions</LinkButton>}{state === "authenticated_storage_unavailable" && <p className="mt-4 text-xs t2">Authentication succeeded, but the Mission store is unavailable. No Mission action is safe to continue.</p>}</Surface>
  </section>;
}
